...
import { useGetScheduleQuery } from "../../../../../store/services";
...
const ScheduleTable: React.FC<ITableProps> = ({
  col,
  setTableData,
  tableData,
  isLoading,
  ...
  console.log(timePickerOpt(),"time picker")

  const checkSlotAvailbilty = (
    availability: IAvailability[],
    currentItem: string,
    index: number,
    endDate?: boolean
  ) => {
    const a = availability.some((av: IAvailability, i) => {
      return (
        (Number(currentItem?.replace(/:/g, "")) >=
          Number(av?.startTime?.replace(/:/g, "")) &&
          Number(currentItem?.replace(/:/g, "")) <=
            Number(av?.endTime?.replace(/:/g, "")) &&
          i !== index) ||
        (endDate &&
          Number(av?.startTime?.replace(/:/g, "")) >=
            Number(currentItem?.replace(/:/g, "")))
